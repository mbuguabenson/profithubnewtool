...
const hasDropdownOption = (block: any, fieldName: string, value: string) => {
    const options = block?.getField?.(fieldName)?.menuGenerator_;
    return Array.isArray(options) && options.some((option: [string, string]) => option?.[1] === value);
};

const waitForDropdownOption = (block: any, fieldName: string, value: string, onReady: () => void, attempt = 0) => {
    ...
    if (attempt >= maxAttempts) {
        console.warn(`URL trade type option "${value}" was not available for field "${fieldName}"`);
    ...
    try {
        ...
        setTimeout(() => {
            try {
                ...
                waitForDropdownOption(tradeTypeBlock, 'TRADETYPE_LIST', pendingUrlTradeType.tradeType, () => {
                    if (!pendingUrlTradeType) {
                        window.Blockly.Events.setGroup(originalGroup);
                        return;
                    }
                    ...
                    if (currentTradeType !== pendingUrlTradeType.tradeType) {
                        ...
                        tradeTypeChangeEvent.group = 'URL_PARAM_UPDATE';
