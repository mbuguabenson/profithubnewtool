...
type BlocklyBlock = {
    ...
    getField?: (fieldName: string) => { menuGenerator_?: [string, string][] } | null;
    getChildByType: (type: string) => BlocklyBlock | null;
};

const hasDropdownOption = (block: BlocklyBlock, fieldName: string, value: string) => {
    const options = block.getField?.(fieldName)?.menuGenerator_;
    return Array.isArray(options) && options.some(option => option?.[1] === value);
};

const waitForDropdownOption = (
    block: BlocklyBlock,
    fieldName: string,
    value: string,
    onReady: () => void,
    ...
    if (hasDropdownOption(block, fieldName, value)) {
        onReady();
        return true;
    }

    if (attempt >= maxAttempts) {
        console.warn(`Trade type option "${value}" was not available for field "${fieldName}"`);
        return false;
    }

    setTimeout(() => waitForDropdownOption(block, fieldName, value, onReady, attempt + 1), 100);
    return true;
};
        ...
        try {
            ...
            const applyTradeType = () => {
                ...
                if (updatedCurrentTradeType !== tradeType) {
                    ...
                    (tradeTypeChangeEvent as any).group = 'TRADE_TYPE_MODAL_UPDATE';
            ...
            if (!waitForDropdownOption(tradeTypeBlock, 'TRADETYPE_LIST', tradeType, applyTradeType)) {
