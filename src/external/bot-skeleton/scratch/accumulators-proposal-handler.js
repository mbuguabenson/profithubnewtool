...
export const handleProposalRequestForAccumulators = instance => {
    ...
    const proposal_request = {
        ...
        amount,
        currency,
        ...getSymbolRequestField(underlying_symbol),
        growth_rate,
    };
    window.Blockly.accumulators_request = removeUndefinedFields(proposal_request);
};
...
export const requestProposalForQS = (input_values, ws) => {
    ...
    return ws
        ?.send(removeUndefinedFields(proposal_request))
        .then(response => {
            if (response.error) {
